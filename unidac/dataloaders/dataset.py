...
Modified in DAC:
Add Metric3D functions for resizing the input image and depth map
"""
        ...
        for op_name in current_ops:
            ...
            if op_meta["geometrical"]:
                ...
                for k, v in gts.items():
                    ...
                    # if the image scale changes, the depth also needs to be scaled
                    if k == 'gt':
                        # print(op_meta["kwargs"]["scale"])
                        gts[k] = Image.fromarray(np.array(gts[k], dtype=np.float32) / op_meta["kwargs"]["scale"])
...
# Key function from Metric3D
def resize_for_input(image, depth, output_shape, intrinsic, canonical_shape, to_canonical_ratio, padding_rgb=[123.675, 116.28, 103.53], mask=None, lat_grid=None, long_grid=None):
    """
    Resize the input.
    ...
    if lat_grid is not None:
        lat_grid = cv2.resize(lat_grid, dsize=(reshape_w, reshape_h), interpolation=cv2.INTER_LINEAR)
        lat_grid = cv2.copyMakeBorder(
            lat_grid, 
            pad_h_half, 
            pad_h - pad_h_half, 
            pad_w_half, 
    ...
    if long_grid is not None:
        long_grid = cv2.resize(long_grid, dsize=(reshape_w, reshape_h), interpolation=cv2.INTER_LINEAR)
        long_grid = cv2.copyMakeBorder(
            long_grid, 
            pad_h_half, 
            pad_h - pad_h_half, 
            pad_w_half, 
    ...
    if mask is not None:
        ...
        mask = cv2.copyMakeBorder(
            ...
            cv2.BORDER_CONSTANT, 
            value=0)
        return image, depth, pad, to_scale_ratio, mask, lat_grid, long_grid
    
    return image, depth, pad, to_scale_ratio, lat_grid, long_grid