...
from unidac.utils.visualization import colorize


class HypersimERPOnlineDataset(BaseDataset):
    min_depth = 0.01
    max_depth = 80
        ...
        with open(os.path.join('splits/hypersim', self.split_file)) as f:
            for line in f:
                ...
                if os.path.exists(img_info["image_filename"]) and os.path.exists(img_info["annotation_filename_depth"]):
                    # due to synthetic data, no errornous camera pose estimation to filter out
    ...
    def __getitem__(self, idx):
        ...
        cam_params = {
            'dataset': 'hypersim',
            'wFOV': point_info['fov_x'],
            'hFOV': point_info['fov_y'],
            'width': point_info['width'],
            'height': point_info['height'],
            'fx': point_info['width']/2/np.tan(point_info['fov_x']/2),
            'fy': point_info['height']/2/np.tan(point_info['fov_y']/2)
        }
