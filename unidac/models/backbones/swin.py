...
class WindowAttention(nn.Module):
    ...
    def __init__(
        ...
        self.relative_position_bias_table = nn.Parameter(
            ...
            # requires_grad=False # TODO: this is one way to to fix the training issue of in-place operation, but not sure if it makes sense not training this
        )  # 2*Wh-1 * 2*Ww-1, nH
    ...
    def __init__(
        ...
        if downsample is not None:
            self.downsample = downsample(
                input_dim=input_dim, output_dim=output_dim, norm_layer=norm_layer
            )
        else:
...
class SwinTransformer(nn.Module):
        ...
        **kwargs,
        ...
        if self.ape:
            ...
            self.absolute_pos_embed = nn.Parameter(
                torch.zeros(
                    1, embed_dims[0], patches_resolution[0], patches_resolution[1]
                )
            )
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(
                input_dim=self.embed_dims[i_layer],
                output_dim=self.embed_dims[min(i_layer + 1, len(self.embed_dims) - 1)],
    ...
    def forward(self, x, **kwargs):
        ...
        for i in range(self.num_layers):
            ...
            out = (
                x_out.view(-1, H, W, self.embed_dims[i])
                .permute(0, 3, 1, 2)
                .contiguous()
            )
            outs.append(out)
