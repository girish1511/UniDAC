...
class DPTHead(nn.Module):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.norm_cfg = dict(type='BatchNorm')#None  # TODO CHECK THIS
        ...
        for _ in range(len(self.in_channels)):
            self.project_list.append(ConvModule(self.channels, self.channels, kernel_size=3, padding=1, norm_cfg=self.norm_cfg))
        ...
        for _ in range(len(self.in_channels)):
            self.conv_depth_list.append(UpConvHead(self.channels, self.n_output_channels))
    ...
    def forward(self, inputs):
        ...
        depth_out_list = []
        out_list = self.forward_features(inputs)
        for i, out in enumerate(out_list):
            depth_out_list.append(self.conv_depth_list[i](self.project_list[i](out)))

        return [d.clamp(-10,10).exp() for d in depth_out_list]
